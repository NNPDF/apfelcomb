...
// ********************************* Basis rotation helpers *************************************
  ...
  double compute_wgt_norm( const appl::grid *g, int const& d, appl::order_id const& order, double const& as, double const& x1, double const& x2)
  {
    ...
    // Factor of alpha_S
    norm*=pow(as*(4.0*M_PI), order.alphs() );
...
//  *************************************** FK Table computation ********************************************
  ...
  double SubGrid::GetQ2max() const
  {
    ...
    double Q2max = 0;
    const int ngrids = applgrid.g->order_ids().size();
    for(int i=0; i<ngrids; i++)  // pto
